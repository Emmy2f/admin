                                ...
                                <tbody>
                                    ...
                                    if ($sqry) {
                                        ...
                                        while ($row = $sqry->fetch_assoc()) {
                                            $qry2 = $con->query("select * from bedAllocationStatus where bedNumber='$row[bedNumber]'");
                                            while ($row2 = $qry2->fetch_assoc()) {
                                                $alloc = $row2['status'];
                                            }
                                            if($alloc==0)
                                            {
                                                $status="<span class='custom-badge status-red'>Vacant</span>";
                                            }
                                            else
                                            {
                                                 $status="<span class='custom-badge status-green'>Occupied</span>";
                                            }
